...
#
# 系统需求:  CentOS 6+, Debian7+, Ubuntu12+
#
...
[[ $EUID -ne 0 ]] && echo -e "${red}Error:${plain} 这个脚本必须以root运行！" && exit 1

[[ -d "/proc/vz" ]] && echo -e "${red}Error:${plain} 你的 VPS 基于 OpenVZ, 不被支持." && exit 1
...
reboot_os() {
    echo
    echo -e "${green}Info:${plain} 系统需要重启."
    read -p "确定要重启系统吗？? [y/n]" is_reboot
    if [[ ${is_reboot} == "y" || ${is_reboot} == "Y" ]]; then
        reboot
    else
        echo -e "${green}Info:${plain} 重启被取消..."
        exit 0
    fi
}
...
install_bbr() {
    ...
    if [ $? -eq 0 ]; then
        echo
        echo -e "${green}Info:${plain} TCP BBR 已经被安装. nothing to do..."
        exit 0
    fi
    check_kernel_version
    if [ $? -eq 0 ]; then
...
echo
echo " URL: https://github.com/youchanlee/bbr"
echo "----------------------------------------"
echo
echo "按任意键开始安装...或者按Ctrl+C取消安装"
char=`get_char`
